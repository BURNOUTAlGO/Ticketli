...
const FilterFields = ({
  ...
  <>
    ...
    <div className="mb-4">
      <label className="block text-xs font-semibold text-[var(--color-text-subtle)] uppercase tracking-wide mb-1.5">
        Price Range: ₹{filterPriceRange[0].toLocaleString()} – ₹
        {filterPriceRange[1].toLocaleString()}
      </label>
      <Slider.Root
        min={0}
        max={10000}
  ...
  const filtered = useMemo(() => {
    let result = tickets.filter((t) => {
      ...
      if (filterClass !== "Any class" && t.trainClass !== filterClass)
        return false;
      if (
        Number(t.price) < filterPriceRange[0] ||
        Number(t.price) > filterPriceRange[1]
      )
        return false;
          ...
          <div className="h-full flex flex-col lg:hidden overflow-y-auto hide-scrollbar">
            ...
            <div className="px-3  sm:px-4 md:px-6 py-4 max-w-3xl md:max-w-none mx-auto w-full">
              {loading ? (
                ...
                <div className="text-center dark:bg-[#0a0a0a] bg-[#f5f5f5]    py-16 sm:py-20 border border-[var(--color-border)] rounded-2xl text-[var(--color-text-subtle)]">
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3 gap-2">
        <div className="min-w-0">
          <h3
            className={`font-bold text-sm font-mono truncate ${isSold ? "text-yellow-900" : "text-green-900"}`}
          >
            {ticket.trainName || "—"}
          </h3>
      ...
      <div className="flex items-center gap-2 sm:gap-3 mb-3">
        <div className="min-w-0">
          <p
            className={`text-lg sm:text-xl font-bold font-mono ${isSold ? "text-yellow-800" : "text-green-900"}`}
          >
            {ticket.departureTime || "—"}
          </p>
        ...
        <div className="flex-1 flex flex-col items-center gap-0.5 min-w-[40px]">
          ...
          <div className="flex items-center w-full gap-1">
            <div
              className={`flex-1 h-px ${isSold ? "bg-yellow-300" : "bg-green-300"}`}
            />
            <span
              className={`text-xs ${isSold ? "text-yellow-500" : "text-green-900"}`}
            >
              →
            </span>
            <div
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="text-right">
            <p
              className={`text-sm sm:text-base font-bold font-mono ${isSold ? "text-yellow-800" : "text-green-900"}`}
            >
              ₹{ticket.price}
            </p>
